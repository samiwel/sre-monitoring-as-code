...
local dashboardInfo (sliKey, slis) =
  {
    ...
    local row = 6,
    dashboard: //[grafana.row.new(title = sliKey)]
      //+
      [
      ...
      [slis[std.objectFields(slis)[elem]].slo_availability_panel { gridPos: { x: 0, y: elem * row, w: 4, h: 6 } }]
      //+
      // Graph panel showing remaining error budget for reportinhg period (30d by default) over
          // selectable number of days
      //[slis[elem].error_budget_panel { gridPos: { x: 4, y: elem * row, w: 10, h: 6 } }]
      //+
      ...
      //[slis[elem].slo_status_panel { gridPos: { x: 14.5, y: elem * row, w: 9, h: 1 } }]
      //+
      // Detail graph for this SLI, generated by metric specific library
      //[slis[elem].graph { gridPos: { x: 14, y: elem * row, w: 10, h: 5 } }]
      //debug(sliKey)
